...
const ImageModel = () => {
    ...
    // --- Helper: extract a usable filename from different possible shapes ---
    const extractFirstPhoto = (photosField) => {
      if (!photosField) return null;

      // If it's already an array
      if (Array.isArray(photosField) && photosField.length > 0) {
        return photosField[0];
      }

      // If it's an object (rare), try first value
      if (typeof photosField === 'object' && photosField !== null) {
        const keys = Object.keys(photosField);
        if (keys.length > 0) return photosField[keys[0]];
        return null;
      }

      // If it's a string that might be a JSON array like '["file.png"]'
      if (typeof photosField === 'string') {
        ...
        try {
          ...
          // not JSON, so assume it's a plain filename string
          return photosField;
        }
      }
    ...
    const sendUnendorsedPosts = async () => {
        ...
        const postsToSend = filteredPosts.slice(0, 100).map(post => ({
            id: post.id,
            category: post.category,
            // image: post.photos, // keep original shape since backend expects it
            image: extractFirstPhoto(post.photos),
            username: post.user ? post.user.name : 'Unknown',
        }));
                ...
                <div className=" flex flex-col  items-center justify-start w-full h-full ">
                    <div className="bg-gray-50 flex flex-row items-center justify-between p-3 sm:p-5  sm:px-5 w-full ">
                        ...
                        <div className="flex flex-row gap-4 items-center justify-center ml-[1px]">
                            ...
                            {userData && (
                                (() => {
                                  // Use helper to support array / stringified array avatar
                                  const avatarFile = extractFirstPhoto(userData?.userData?.photo);
                                  const avatarSrc = avatarFile ? `${API_URL}/image/${avatarFile}` : APP_PATH + "images/default-avatar.svg";
                                  return (
                                    <Img
                                      className=" sm:w-[58px] sm:h-[52px] md:w-[58px] md:h-[52px] lg:w-[58px] lg:h-[58px]  w-14 h-12 rounded-full object-cover object-top  "
                                      src={avatarSrc}
                        ...
                        <div className={`scroller relative mt-5 w-full h-96 bg-blue-300  post-container ${filteredPosts.length === 0 ? "overflow-hidden" : "overflow-auto"}`}>
                            {filteredPosts.length === 0 ? (
                                ...
                                <table className="w-full border-collapse border-2">
                                    ...
                                    <tbody className=" ">
                                        {filteredPosts.map((post) => {
                                            const firstPhoto = extractFirstPhoto(post.photos);
                                            const imgSrc = firstPhoto ? `${API_URL}/image/${firstPhoto}` : APP_PATH + "images/nopost.svg";
                                            let formattedDate = "Unknown";
                                            try {
                                              if (post.Date) formattedDate = format(new Date(post.Date), "dd-MM-yyyy");
                                            } catch (e) {
                                            ...
                                            return (
                                              <tr key={post.id} className=" border">
                                                ...
                                                <td className="border p-3 text-center">
                                                    <img className='rounded max-w-[120px] max-h-[80px] object-cover mx-auto' src={imgSrc} alt="post image" />
                                                </td>
                                              </tr>
                                            )
                                        })}

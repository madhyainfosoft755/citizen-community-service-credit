...
const ProfileForUser = () => {
  ...
  const [usersWithMostPostsInSixMonths, setUsersWithMostPostsInSixMonths] =
    useState([]);
  ...
  const [showCertificateError, setShowCertificateError] = useState(false);


  const hasEnoughTime = (timeString) => {
    const [hours, minutes] = timeString.split(":").map(Number);
    return hours >= 1 || (hours === 0 && minutes >= 60);
  };

  const handleCertificateClick = () => {
    if (totalTime && hasEnoughTime(totalTime.split(" ")[0])) {
      setIsPopupVisible(true);
    } else {
      setShowCertificateError(true);
      setTimeout(() => setShowCertificateError(false), 3000);
    }
  };
  ...
  const MostPostsInYear = async () => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/activity/getUsersWithMostPostsInYear`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        ...
        setUsersWithMostPostsInYear(
          Array.isArray(topUserNames) ? topUserNames : []
        );
      } else {
        console.error(
          "Error fetching users with most posts in year:",
          response.status
        );
        setError(
          "An error occurred while fetching users with most posts in year."
        );
      }
    } catch (error) {
  ...
  const fetchMostPostsInSixMonths = async () => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/activity/getUsersWithMostPostsInSixMonths`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        const { topUserNames } = await response.json();
        setUsersWithMostPostsInSixMonths(
          Array.isArray(topUserNames) ? topUserNames : []
        );
      } else {
        console.error(
          "Error fetching users with most posts in six months:",
          response.status
        );
        setError(
          "An error occurred while fetching users with most posts in six months."
        );
      }
    } catch (error) {
      console.error(
  ...
  const fetchMostPostsInQuater = async () => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/activity/getUsersWithMostPostsInQuater`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const fetchMostPostsInMonth = async () => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/activity/getUsersWithMostPostsInMonth`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        const { topUserNames } = await response.json();
        setUsersWithMostPostsInMonth(
          Array.isArray(topUserNames) ? topUserNames : []
        );
      } else {
        console.error(
          "Error fetching users with most posts in month:",
          response.status
        );
        setError(
          "An error occurred while fetching users with most posts in month."
        );
      }
    } catch (error) {
        ...
        <div className=" relative w-4/12 h-full sm:w-full sm:h-full md:w-3/4 md:h-full  lg:w-3/4 lg:h-full  flex flex-col items-center  justify-center border-[1px]  rounded-lg sm:rounded-none overflow-hidden">
          <div className=" flex flex-col  items-center justify-center w-full h-full ">
            <div className="bg-gray-50 flex flex-row items-center justify-between p-2 sm:px-2   w-full ">
              ...
              <Button
                onClick={direct}
                className="text-blue-500 bg-white-A700 px-3 py-2 rounded-full text-xs font-semibold cursor-pointer"
              >{`${totalTime || 0} Hrs | ${
            ...
            <div className=" w-full h-full  pt-3 ">
              <div className=" w-full h-3/5 flex flex-wrap items-center justify-between pl-4 pr-4 pt-1 pb-1">
                <div
                  className="w-[48%] rounded-lg bg-[#ffffff] border-[1px]  text-[#6b84ff] h-1/5 flex  items-center justify-center font-semibold cursor-pointer text-center"
                  onClick={() => {
                    navigate("../create");
                  }}
                >
                  <h1>
                    Create Activity <FontAwesomeIcon icon={faUser} />
                  </h1>
                </div>
                <div
                  className="w-[48%] rounded-lg bg-[#ffffff] border-[1px]  text-[#6b84ff] h-1/5 flex  items-center justify-center font-semibold cursor-pointer text-center"
                ...
                >
                  <h1>
                    Endorse <FontAwesomeIcon icon={faUser} />
                  </h1>
                </div>
                <div
                  className="w-[48%] rounded-lg bg-[#ffffff] border-[1px]  text-[#6b84ff] h-1/5 flex  items-center justify-center font-semibold cursor-pointer text-center"
                ...
                >
                  <h1>
                    Activities <FontAwesomeIcon icon={faUser} />
                  </h1>
                </div>
                <div
                  className="w-[48%] rounded-lg bg-[#ffffff] border-[1px]  text-[#6b84ff] h-1/5 flex  items-center justify-center font-semibold cursor-pointer text-center"
                ...
                >
                  <h1>
                    Edit Profile <FontAwesomeIcon icon={faUser} />
                  </h1>
                </div>
                <div
                  className={` w-[48%] rounded-lg bg-[#ffffff] border-[1px] ${totalTime && hasEnoughTime(totalTime.split(' ')[0]) ? 'text-[#6b84ff]' : 'text-[#a9aaaf]'} h-1/5 flex flex-shrink-2 items-center justify-center font-semibold cursor-pointer text-center`}
                  onClick={handleCertificateClick}
                >
                  {showCertificateError ? (
                    <small className=" text-red-500 text-center mt-2 tracking-tighter">
                      You must have at least 1 hour of activity time to <br />{" "}
                      download the certificate.
                    </small>
                  ) : (
                    <h1>
                      Certificate <FontAwesomeIcon icon={faDownload} />
                    </h1>
                  )}
                </div>
                <div
                  className="w-[48%] rounded-lg bg-[#ffffff] border-[1px]  text-[#6b84ff] h-1/5 flex flex-shrink-2 items-center justify-center font-semibold cursor-pointer text-center"
                  onClick={() => {
                    navigate("../user-report");
                  }}
                >
                  <h1>
                    Genrate Reports <FontAwesomeIcon icon={faFileAlt} />
                  </h1>
                </div>
                <div
                  className="w-[48%] rounded-lg bg-[#ffffff] border-[1px]  text-[#a9aaaf] h-1/5 flex flex-shrink-2 items-center justify-center font-semibold cursor-pointer text-center"
                  onClick={() => {}}
                >
                  <h1>
                    Group <FontAwesomeIcon icon={faComment} />
                  </h1>
                </div>
              </div>
